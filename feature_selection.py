...
from data_prep import y_smote_sc, X_smote_sc
...
#       Nog implementeren?
# 4) Bin on WOE value: IV criteria
#       See woe_iv.py
...
vif_data['Features'] = X_smote_sc.columns
vif_data['VIF'] = [variance_inflation_factor(X_smote_sc.values, i) for i in range(len(X_smote_sc.columns))]
print(vif_data)

# Remove highly related variables
# Nog checken waarom 2.8
while any(vif_data['VIF'] >= 2.8):
    i = vif_data['VIF'].idxmax()
    X_smote_sc.drop(X_smote_sc.columns[i], axis=1, inplace=True)
    X_smote_sc.reset_index(drop=True)
    vif_data = pd.DataFrame()
    vif_data['Features'] = X_smote_sc.columns
    vif_data['VIF'] = [variance_inflation_factor(X_smote_sc.values, i) for i in range(len(X_smote_sc.columns))]
print(vif_data)
